...
void object3(point& camera_world_position, point& camera_translate, point& camera_vision_z, point& camera_vision_x, point& camera_vision_y)
{
	vector<point> point_world, point_camera;
	vector<point_2d> point_pixel;
	const float m = 10;
	float d = rand() % 10 + 1;
		point_world.push_back({ m+d, d, d });
		point_world.push_back({ m+d, d, m +d});
		point_world.push_back({ d, d, m+d });
		point_world.push_back({ d, d, d});
		point_world.push_back({ m+d, m+d, d });
		point_world.push_back({ m+d, m+d, m+d });
		point_world.push_back({ d, m+d, m+d });
		point_world.push_back({ d, m+d, d });
	world_to_camera(point_world, point_camera, camera_translate, camera_vision_z, camera_vision_x, camera_vision_y);
	camera_to_screen(point_camera, point_pixel, point_world, camera_world_position);
	setlinecolor(RED);
	draw(point_pixel);
...
sky(camera_world_position, camera_translate, camera_vision, camera_vision_z, camera_vision_x, camera_vision_y);
		...
		object3(camera_world_position, camera_translate, camera_vision_z, camera_vision_x, camera_vision_y);
		//绘制光标 
		draw_sign();
		//fov
		{
